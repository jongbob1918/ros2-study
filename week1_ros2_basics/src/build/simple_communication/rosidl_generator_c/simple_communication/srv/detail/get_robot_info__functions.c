// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from simple_communication:srv/GetRobotInfo.idl
// generated code does not contain a copyright notice
#include "simple_communication/srv/detail/get_robot_info__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"

bool
simple_communication__srv__GetRobotInfo_Request__init(simple_communication__srv__GetRobotInfo_Request * msg)
{
  if (!msg) {
    return false;
  }
  // structure_needs_at_least_one_member
  return true;
}

void
simple_communication__srv__GetRobotInfo_Request__fini(simple_communication__srv__GetRobotInfo_Request * msg)
{
  if (!msg) {
    return;
  }
  // structure_needs_at_least_one_member
}

bool
simple_communication__srv__GetRobotInfo_Request__are_equal(const simple_communication__srv__GetRobotInfo_Request * lhs, const simple_communication__srv__GetRobotInfo_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // structure_needs_at_least_one_member
  if (lhs->structure_needs_at_least_one_member != rhs->structure_needs_at_least_one_member) {
    return false;
  }
  return true;
}

bool
simple_communication__srv__GetRobotInfo_Request__copy(
  const simple_communication__srv__GetRobotInfo_Request * input,
  simple_communication__srv__GetRobotInfo_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // structure_needs_at_least_one_member
  output->structure_needs_at_least_one_member = input->structure_needs_at_least_one_member;
  return true;
}

simple_communication__srv__GetRobotInfo_Request *
simple_communication__srv__GetRobotInfo_Request__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  simple_communication__srv__GetRobotInfo_Request * msg = (simple_communication__srv__GetRobotInfo_Request *)allocator.allocate(sizeof(simple_communication__srv__GetRobotInfo_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(simple_communication__srv__GetRobotInfo_Request));
  bool success = simple_communication__srv__GetRobotInfo_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
simple_communication__srv__GetRobotInfo_Request__destroy(simple_communication__srv__GetRobotInfo_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    simple_communication__srv__GetRobotInfo_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
simple_communication__srv__GetRobotInfo_Request__Sequence__init(simple_communication__srv__GetRobotInfo_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  simple_communication__srv__GetRobotInfo_Request * data = NULL;

  if (size) {
    data = (simple_communication__srv__GetRobotInfo_Request *)allocator.zero_allocate(size, sizeof(simple_communication__srv__GetRobotInfo_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = simple_communication__srv__GetRobotInfo_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        simple_communication__srv__GetRobotInfo_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
simple_communication__srv__GetRobotInfo_Request__Sequence__fini(simple_communication__srv__GetRobotInfo_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      simple_communication__srv__GetRobotInfo_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

simple_communication__srv__GetRobotInfo_Request__Sequence *
simple_communication__srv__GetRobotInfo_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  simple_communication__srv__GetRobotInfo_Request__Sequence * array = (simple_communication__srv__GetRobotInfo_Request__Sequence *)allocator.allocate(sizeof(simple_communication__srv__GetRobotInfo_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = simple_communication__srv__GetRobotInfo_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
simple_communication__srv__GetRobotInfo_Request__Sequence__destroy(simple_communication__srv__GetRobotInfo_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    simple_communication__srv__GetRobotInfo_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
simple_communication__srv__GetRobotInfo_Request__Sequence__are_equal(const simple_communication__srv__GetRobotInfo_Request__Sequence * lhs, const simple_communication__srv__GetRobotInfo_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!simple_communication__srv__GetRobotInfo_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
simple_communication__srv__GetRobotInfo_Request__Sequence__copy(
  const simple_communication__srv__GetRobotInfo_Request__Sequence * input,
  simple_communication__srv__GetRobotInfo_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(simple_communication__srv__GetRobotInfo_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    simple_communication__srv__GetRobotInfo_Request * data =
      (simple_communication__srv__GetRobotInfo_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!simple_communication__srv__GetRobotInfo_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          simple_communication__srv__GetRobotInfo_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!simple_communication__srv__GetRobotInfo_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `robot_name`
// Member `status`
#include "rosidl_runtime_c/string_functions.h"

bool
simple_communication__srv__GetRobotInfo_Response__init(simple_communication__srv__GetRobotInfo_Response * msg)
{
  if (!msg) {
    return false;
  }
  // robot_name
  if (!rosidl_runtime_c__String__init(&msg->robot_name)) {
    simple_communication__srv__GetRobotInfo_Response__fini(msg);
    return false;
  }
  // status
  if (!rosidl_runtime_c__String__init(&msg->status)) {
    simple_communication__srv__GetRobotInfo_Response__fini(msg);
    return false;
  }
  // battery_level
  return true;
}

void
simple_communication__srv__GetRobotInfo_Response__fini(simple_communication__srv__GetRobotInfo_Response * msg)
{
  if (!msg) {
    return;
  }
  // robot_name
  rosidl_runtime_c__String__fini(&msg->robot_name);
  // status
  rosidl_runtime_c__String__fini(&msg->status);
  // battery_level
}

bool
simple_communication__srv__GetRobotInfo_Response__are_equal(const simple_communication__srv__GetRobotInfo_Response * lhs, const simple_communication__srv__GetRobotInfo_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // robot_name
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->robot_name), &(rhs->robot_name)))
  {
    return false;
  }
  // status
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->status), &(rhs->status)))
  {
    return false;
  }
  // battery_level
  if (lhs->battery_level != rhs->battery_level) {
    return false;
  }
  return true;
}

bool
simple_communication__srv__GetRobotInfo_Response__copy(
  const simple_communication__srv__GetRobotInfo_Response * input,
  simple_communication__srv__GetRobotInfo_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // robot_name
  if (!rosidl_runtime_c__String__copy(
      &(input->robot_name), &(output->robot_name)))
  {
    return false;
  }
  // status
  if (!rosidl_runtime_c__String__copy(
      &(input->status), &(output->status)))
  {
    return false;
  }
  // battery_level
  output->battery_level = input->battery_level;
  return true;
}

simple_communication__srv__GetRobotInfo_Response *
simple_communication__srv__GetRobotInfo_Response__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  simple_communication__srv__GetRobotInfo_Response * msg = (simple_communication__srv__GetRobotInfo_Response *)allocator.allocate(sizeof(simple_communication__srv__GetRobotInfo_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(simple_communication__srv__GetRobotInfo_Response));
  bool success = simple_communication__srv__GetRobotInfo_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
simple_communication__srv__GetRobotInfo_Response__destroy(simple_communication__srv__GetRobotInfo_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    simple_communication__srv__GetRobotInfo_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
simple_communication__srv__GetRobotInfo_Response__Sequence__init(simple_communication__srv__GetRobotInfo_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  simple_communication__srv__GetRobotInfo_Response * data = NULL;

  if (size) {
    data = (simple_communication__srv__GetRobotInfo_Response *)allocator.zero_allocate(size, sizeof(simple_communication__srv__GetRobotInfo_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = simple_communication__srv__GetRobotInfo_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        simple_communication__srv__GetRobotInfo_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
simple_communication__srv__GetRobotInfo_Response__Sequence__fini(simple_communication__srv__GetRobotInfo_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      simple_communication__srv__GetRobotInfo_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

simple_communication__srv__GetRobotInfo_Response__Sequence *
simple_communication__srv__GetRobotInfo_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  simple_communication__srv__GetRobotInfo_Response__Sequence * array = (simple_communication__srv__GetRobotInfo_Response__Sequence *)allocator.allocate(sizeof(simple_communication__srv__GetRobotInfo_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = simple_communication__srv__GetRobotInfo_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
simple_communication__srv__GetRobotInfo_Response__Sequence__destroy(simple_communication__srv__GetRobotInfo_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    simple_communication__srv__GetRobotInfo_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
simple_communication__srv__GetRobotInfo_Response__Sequence__are_equal(const simple_communication__srv__GetRobotInfo_Response__Sequence * lhs, const simple_communication__srv__GetRobotInfo_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!simple_communication__srv__GetRobotInfo_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
simple_communication__srv__GetRobotInfo_Response__Sequence__copy(
  const simple_communication__srv__GetRobotInfo_Response__Sequence * input,
  simple_communication__srv__GetRobotInfo_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(simple_communication__srv__GetRobotInfo_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    simple_communication__srv__GetRobotInfo_Response * data =
      (simple_communication__srv__GetRobotInfo_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!simple_communication__srv__GetRobotInfo_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          simple_communication__srv__GetRobotInfo_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!simple_communication__srv__GetRobotInfo_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `info`
#include "service_msgs/msg/detail/service_event_info__functions.h"
// Member `request`
// Member `response`
// already included above
// #include "simple_communication/srv/detail/get_robot_info__functions.h"

bool
simple_communication__srv__GetRobotInfo_Event__init(simple_communication__srv__GetRobotInfo_Event * msg)
{
  if (!msg) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__init(&msg->info)) {
    simple_communication__srv__GetRobotInfo_Event__fini(msg);
    return false;
  }
  // request
  if (!simple_communication__srv__GetRobotInfo_Request__Sequence__init(&msg->request, 0)) {
    simple_communication__srv__GetRobotInfo_Event__fini(msg);
    return false;
  }
  // response
  if (!simple_communication__srv__GetRobotInfo_Response__Sequence__init(&msg->response, 0)) {
    simple_communication__srv__GetRobotInfo_Event__fini(msg);
    return false;
  }
  return true;
}

void
simple_communication__srv__GetRobotInfo_Event__fini(simple_communication__srv__GetRobotInfo_Event * msg)
{
  if (!msg) {
    return;
  }
  // info
  service_msgs__msg__ServiceEventInfo__fini(&msg->info);
  // request
  simple_communication__srv__GetRobotInfo_Request__Sequence__fini(&msg->request);
  // response
  simple_communication__srv__GetRobotInfo_Response__Sequence__fini(&msg->response);
}

bool
simple_communication__srv__GetRobotInfo_Event__are_equal(const simple_communication__srv__GetRobotInfo_Event * lhs, const simple_communication__srv__GetRobotInfo_Event * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__are_equal(
      &(lhs->info), &(rhs->info)))
  {
    return false;
  }
  // request
  if (!simple_communication__srv__GetRobotInfo_Request__Sequence__are_equal(
      &(lhs->request), &(rhs->request)))
  {
    return false;
  }
  // response
  if (!simple_communication__srv__GetRobotInfo_Response__Sequence__are_equal(
      &(lhs->response), &(rhs->response)))
  {
    return false;
  }
  return true;
}

bool
simple_communication__srv__GetRobotInfo_Event__copy(
  const simple_communication__srv__GetRobotInfo_Event * input,
  simple_communication__srv__GetRobotInfo_Event * output)
{
  if (!input || !output) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__copy(
      &(input->info), &(output->info)))
  {
    return false;
  }
  // request
  if (!simple_communication__srv__GetRobotInfo_Request__Sequence__copy(
      &(input->request), &(output->request)))
  {
    return false;
  }
  // response
  if (!simple_communication__srv__GetRobotInfo_Response__Sequence__copy(
      &(input->response), &(output->response)))
  {
    return false;
  }
  return true;
}

simple_communication__srv__GetRobotInfo_Event *
simple_communication__srv__GetRobotInfo_Event__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  simple_communication__srv__GetRobotInfo_Event * msg = (simple_communication__srv__GetRobotInfo_Event *)allocator.allocate(sizeof(simple_communication__srv__GetRobotInfo_Event), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(simple_communication__srv__GetRobotInfo_Event));
  bool success = simple_communication__srv__GetRobotInfo_Event__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
simple_communication__srv__GetRobotInfo_Event__destroy(simple_communication__srv__GetRobotInfo_Event * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    simple_communication__srv__GetRobotInfo_Event__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
simple_communication__srv__GetRobotInfo_Event__Sequence__init(simple_communication__srv__GetRobotInfo_Event__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  simple_communication__srv__GetRobotInfo_Event * data = NULL;

  if (size) {
    data = (simple_communication__srv__GetRobotInfo_Event *)allocator.zero_allocate(size, sizeof(simple_communication__srv__GetRobotInfo_Event), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = simple_communication__srv__GetRobotInfo_Event__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        simple_communication__srv__GetRobotInfo_Event__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
simple_communication__srv__GetRobotInfo_Event__Sequence__fini(simple_communication__srv__GetRobotInfo_Event__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      simple_communication__srv__GetRobotInfo_Event__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

simple_communication__srv__GetRobotInfo_Event__Sequence *
simple_communication__srv__GetRobotInfo_Event__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  simple_communication__srv__GetRobotInfo_Event__Sequence * array = (simple_communication__srv__GetRobotInfo_Event__Sequence *)allocator.allocate(sizeof(simple_communication__srv__GetRobotInfo_Event__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = simple_communication__srv__GetRobotInfo_Event__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
simple_communication__srv__GetRobotInfo_Event__Sequence__destroy(simple_communication__srv__GetRobotInfo_Event__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    simple_communication__srv__GetRobotInfo_Event__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
simple_communication__srv__GetRobotInfo_Event__Sequence__are_equal(const simple_communication__srv__GetRobotInfo_Event__Sequence * lhs, const simple_communication__srv__GetRobotInfo_Event__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!simple_communication__srv__GetRobotInfo_Event__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
simple_communication__srv__GetRobotInfo_Event__Sequence__copy(
  const simple_communication__srv__GetRobotInfo_Event__Sequence * input,
  simple_communication__srv__GetRobotInfo_Event__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(simple_communication__srv__GetRobotInfo_Event);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    simple_communication__srv__GetRobotInfo_Event * data =
      (simple_communication__srv__GetRobotInfo_Event *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!simple_communication__srv__GetRobotInfo_Event__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          simple_communication__srv__GetRobotInfo_Event__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!simple_communication__srv__GetRobotInfo_Event__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
